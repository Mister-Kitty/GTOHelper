...
public class PT4SessionDM extends DataManagerBase implements ISessionDM {
    ...
    private final String sessionSqlQuery = "SELECT (cash_table_session_summary.id_session) as \"id_sessions\", \n" +
            ...
            "(cash_table_session_summary.id_session) as \"id_session\", (sum((case when(cash_hand_player_statistics.id_hand > 0) then  1 else  0 end))) as \"cnt_hands\", \n" +
            ...
            "(sum(cash_hand_player_statistics.amt_payout)) as \"amt_payout\", (sum((case when(cash_hand_player_statistics.flg_vpip) then  1 else  0 end))) as \"cnt_vpip\",\n" +
            "(sum((case when(lookup_actions_p.action = '') then  1 else  0 end))) as \"cnt_walks\", (sum((case when(cash_hand_player_statistics.cnt_p_raise > 0) then  1 else  0 end))) as \"cnt_pfr\",\n" +
            ...
            "(sum((case when(cash_hand_player_statistics.flg_f_bet) then  1 else  0 end))) as \"cnt_f_bet\", (sum(cash_hand_player_statistics.cnt_f_raise)) as \"cnt_f_raise\", \n" +
            "(sum((case when(cash_hand_player_statistics.flg_t_bet) then  1 else  0 end))) as \"cnt_t_bet\", (sum(cash_hand_player_statistics.cnt_t_raise)) as \"cnt_t_raise\", \n" +
            "(sum((case when(cash_hand_player_statistics.flg_r_bet) then  1 else  0 end))) as \"cnt_r_bet\", (sum(cash_hand_player_statistics.cnt_r_raise)) as \"cnt_r_raise\", \n" +
            "(sum(cash_hand_player_statistics.cnt_f_call )) as \"cnt_f_call\", (sum(cash_hand_player_statistics.cnt_t_call )) as \"cnt_t_call\", \n" +
            "(sum(cash_hand_player_statistics.cnt_r_call )) as \"cnt_r_call\", (sum((case when(cash_hand_player_statistics.flg_f_fold) then  1 else  0 end))) as \"cnt_f_fold\", \n" +
            ...
            "(timezone('UTC',  cash_table_session_summary.date_end  + INTERVAL '0 HOURS')) as \"session_date_end\", (sum(cash_hand_player_statistics.amt_won)) as \"amt_won\" \n" +
            "\n" +
